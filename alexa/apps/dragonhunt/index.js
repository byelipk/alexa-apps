...
app.launch(function(req, res) {
  ...
  // Welcome the user.
  res.say(
    "A large " + dragon.type + " dragon approaches! What do you want to do?"
  );
...
app.intent(
  ...
  {
    slots: {},
    utterances: ["{look|search|find} {a|} {weapon|sword|peace maker}"]
  },
  function(req, res) {
...
app.intent(
  "attack",
  {
    ...
    utterances: [
      "{attack|fight|hit|use} {lance|weapon} on {-|DragonType} dragon"
    ]
  },
  function(req, res) {
    if (!req.session("hasWeapon")) {
      ...
      if (dragon.type.toLowerCase() != req.slot("DragonType").toLowerCase()) {
        ...
        res.say(
          "I don't see a " +
            req.slot("DragonType") +
            " dragon anywhere! Just a " + dragon.type + " one."
        );
        res.shouldEndSession(false);
      } else {
        ...
        if (dragon.hp <= 0) {
          res.say(
            "You have slain the mighty " +
              dragon.type + " dragon! Congratulations!"
          );
          ...
          res.say(
            "You attack the " +
              dragon.type +
              " dragon with your lance! It has " + dragon.hp + " HP left!"
          );
          res.shouldEndSession(false);
        }
      }
